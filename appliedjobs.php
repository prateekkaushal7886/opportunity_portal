...
<body>
	...
	<div class="container">
		<p class="menuheading animated">Here are the jobs you applied for</p>
		...
		if(sizeof($jobs) != 0) {
			for ($i=0; $i < sizeof($jobs); $i++) {
				if(in_array($jobs[$i]['id'], $job_ids)) { 
					...
					<td>'.date("d-m-y", strtotime($jobs[$i]['applyby'])).'</td>
					</tr>
					</tbody>
					</table>
					</div>
					...
					<a class="btn modal-trigger animated apply_btn" href="#modal" id="'.$jobs[$i]['id'].'" style="margin-left: 10px; margin-top: 5px;">Applied</a>
					</div>
					</div>
					</div>
					</div>
